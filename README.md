...
실행 코드가 아니라 **번역 자산**이다. 확정 문구 TM 67건, 공지 빈출 고유명사 18건, 표기 규칙 3건, 그리고 애매할 때의 판단 기준.
...
```bash
git clone https://github.com/<owner>/pubg-announcements.git
...
| | 파일 | 맡는 것 |
|---|---|---|
| 1 | `glossary/announcements.json` | **확정 문장 TM (67)** — 있으면 그대로. 재번역 금지 |
| 2 | `references/notation.json` | **패턴 규칙 (3)** — 날짜·기간 범위·title case. 문자열이 아니라 형태라 TM 으로 못 박는 것 |
| 3 | `glossary/proper_nouns.json` | 공지 빈출 고유명사 (18). 인게임 표기가 정본 |
...
4. 등록 후 `check-counts.sh` 로 수치를 맞춘다.
...
각 항목의 근거를 공지 제목·발행 연월로 채우는 작업이 남아 있다 — `references/sources.md` 의 TODO.
