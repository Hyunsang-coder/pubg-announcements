...
**이 저장소는 한국어 → 영어만 다룬다.** 92건 전부 `source_lang: ko` / `target_lang: en` 이고, 이건 항목별 속성이 아니라 저장소 불변식이다.
...
## 2. 고유명사 (22개) — 아래 표가 전량. 원본은 `glossary/proper_nouns.json`

발행 공지에 실제 등장한 인게임 고유명사만 담았다. 22건뿐이라 **파일을 열 필요 없이 여기서 바로 쓴다.**
